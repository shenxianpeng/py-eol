...
def test_main_check_self(monkeypatch):
	...
	monkeypatch.setattr(cli_mod, "check_self", lambda output_json=False: (_ for _ in ()).throw(SystemExit(0)))
	with pytest.raises(SystemExit) as e:
		cli_mod.main()
	assert e.value.code == 0
...
def test_main_list(monkeypatch, capsys):
	...
	monkeypatch.setattr(cli_mod, "list_supported_versions", lambda output_json=False: (_ for _ in ()).throw(SystemExit(0)))
	with pytest.raises(SystemExit) as e:
		cli_mod.main()
	assert e.value.code == 0
...
def test_main_versions(monkeypatch, capsys):
	...
	monkeypatch.setattr(cli_mod, "check_versions", lambda versions, output_json=False: (_ for _ in ()).throw(SystemExit(0)))
	with pytest.raises(SystemExit) as e:
		cli_mod.main()
	assert e.value.code == 0
...
def test_main_default(monkeypatch, capsys):
	...
	monkeypatch.setattr(cli_mod.argparse, "ArgumentParser", lambda *a, **k: DummyParser())
	try:
		with pytest.raises(SystemExit) as e:
			cli_mod.main()
		out = capsys.readouterr().out
...
def test_check_versions_supported(monkeypatch, capsys):
	monkeypatch.setattr(cli_mod, "get_eol_date", lambda v: sys.modules["datetime"].date(2099, 1, 1))
...
def test_check_versions_eol(monkeypatch, capsys):
	monkeypatch.setattr(cli_mod, "get_eol_date", lambda v: sys.modules["datetime"].date(2000, 1, 1))
...
def test_check_self(monkeypatch):
	monkeypatch.setattr(cli_mod, "check_versions", lambda v, output_json=False: (_ for _ in ()).throw(SystemExit(0)))
	with pytest.raises(SystemExit):
		cli_mod.check_self()
